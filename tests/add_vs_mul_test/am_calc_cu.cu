...
template<typename T>
__global__ void vec_mul_kernel( T * a, T * b, T * c, int size )
{
   int bx = blockIdx.x;
   int tx = threadIdx.x;
...
template<typename T>
time_res_t vec_mul( const T * a, const T * b, T * c, int block_size, int size )
 {
   T   * d_A = NULL
     , * d_B = NULL
     , * d_C = NULL;
   ...
   vec_mul_kernel<T><<< grid, threads >>>(d_A, d_B, d_C, size);
...
time_res_t va_calc( int size, const T * a, const T * b, T * c )
{
   ...
   time_res_t duration = vec_mul(a, b, c, block_size, size);
...
time_res_t vm_calc( int size, const float * a, const float * b, float * c )
{
   return vm_calc<float>(size, a, b, c);
}

time_res_t vm_calc( int size, const double * a, const double * b, double * c )
{
   return vm_calc<double>(size, a, b, c);
}

time_res_t vm_calc( int size, const int * a, const int * b, int * c )
{
   return vm_calc<int>(size, a, b, c);
}
